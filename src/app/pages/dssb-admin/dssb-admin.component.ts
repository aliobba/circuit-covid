...
@Component({
  selector: 'app-dssb-admin',
  templateUrl: './dssb-admin.component.html',
  styleUrls: ['./dssb-admin.component.css']
})
export class DssbAdminComponent implements OnInit {
  ...
  userstorage: string;
  rolecurr: string;
  constructor(private fb: FormBuilder, private userService:  UsersService, private route: ActivatedRoute, private router: Router, private doctorService:  DoctorService) {
  ...
  ajouter() {
    ...
    this.loading = true;
    //this.userService.createUser(this.f.nom.value, this.f.prenom.value, this.f.username.value, this.f.pwd.value, this.f.rl.value, this.f.gouvernorat.value, this.f.delegation.value , this.f.Adress.value , this.f.codepost.value)
      //.subscribe( data => {
